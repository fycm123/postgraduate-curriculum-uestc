...
LRESULT CALLBACK WindowProc(HWND hwnd,UINT uMsg,WPARAM wParam,LPARAM lParam ){
	...
	switch(uMsg){
		case WM_CREATE:
			hButtonLogin=CreateWindow(TEXT("button"),TEXT("Login"),WS_CHILD|WS_VISIBLE|BS_PUSHBUTTON,
		...
		rect.right=50;
		rect.bottom=30;
		DrawText(hdc,TEXT("UserName:"),-1,&rect,DT_SINGLELINE|DT_CENTER|DT_VCENTER);
		rect.left=150;
		rect.top=160;
		rect.right=50;
		rect.bottom=30;
		DrawText(hdc,TEXT("Password:"),-1,&rect,DT_SINGLELINE|DT_CENTER|DT_VCENTER);
		return 0;
		case WM_COMMAND:
			...
			if(strcmp(userName,"yangqing")==0&&strcmp(password,"201822090316")==0){
				MessageBox(hwnd,TEXT("Success!"),TEXT("Congratulations"),MB_OK);
			}else{
				MessageBox(hwnd,TEXT("Failed!"),TEXT("Sorry"),MB_OK);
			}
		}
			return 0;
		case WM_DESTROY:
